/*
 * For UCF CS II - Single Linked list code example & lightweight test harness.
 *
 *    This code, reviewed in class on January 15 performs the following 
 * functions in a rudimentary way. 
 *
 *    Use this code sample to familiarize yourself with the following:
 *  - uploading source code to Eustis (scp source/file destination)
 *  - compiling Java on Eustis (javac java filename.java)
 *  - execute said program on Eustis (java filename)
 *
 *    This code is missing one function - search.
 * 
 *    Therefore your goal is to add a search function that searches 
 * based on:
 *  - finding matching data
 *      ----OR----
 *  - finding the i^th element in the list
 *  - in the event that either the data can't be found or the i^th 
 *    element cannot be found - advise the user of said failute.
 */

/**
 *
 * @author michaelmcalpin
 */
